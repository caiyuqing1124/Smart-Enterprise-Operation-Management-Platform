...
import {
  efficiencyMetrics as historicalEfficiencyMetrics,
  industryContribution as historicalIndustryContribution,
  operationRisks as historicalOperationRisks,
  operationSnapshots,
  regionPerformance as historicalRegionPerformance,
...
const isCurrentPeriod = computed(() => period.value === '2026-09')
const displayMetrics = computed(() => isCurrentPeriod.value ? businessMetrics.currentMetrics.value : snapshot.value.metrics)
const displayIndustryContribution = computed(() => isCurrentPeriod.value ? businessMetrics.industryContribution.value : historicalIndustryContribution)
const displayRegionPerformance = computed(() => isCurrentPeriod.value ? businessMetrics.regionPerformance.value : historicalRegionPerformance)
const displayEfficiencyMetrics = computed(() => isCurrentPeriod.value ? businessMetrics.efficiencyMetrics.value : historicalEfficiencyMetrics)
const displayRisks = computed(() => isCurrentPeriod.value ? businessMetrics.operationRisks.value : historicalOperationRisks)
...
const industryOption = computed(() => ({
  ...
  series: [{ type: 'pie', radius: ['47%', '70%'], center: ['36%', '52%'], label: { show: false }, data: displayIndustryContribution.value }],
}))
...
const regionOption = computed(() => ({
  ...
  yAxis: { type: 'category', data: [...displayRegionPerformance.value].reverse().map((item) => item.name), axisLine: { show: false }, axisTick: { show: false }, axisLabel: { color: '#93a8c5' } },
  series: [{ type: 'bar', barWidth: 13, data: [...displayRegionPerformance.value].reverse().map((item) => item.revenue), itemStyle: { borderRadius: [0, 7, 7, 0] } }],
}))
...
function exportData() {
  const rows = [['区域', '合同金额（万元）', '收入贡献率', '回款率'], ...displayRegionPerformance.value.map((item) => [item.name, item.revenue, `${item.share}%`, `${item.targetRate}%`])]
...
<template>
  <div ref="cockpitRef" class="cockpit-page">
    <header class="cockpit-header">
      <div><span>ENTERPRISE OPERATION COCKPIT</span><h1>经营驾驶舱</h1><p>{{ isCurrentPeriod ? '当前周期已联动客户、项目与财务数据' : '历史周期经营快照' }}</p></div>
    ...
    <section class="cockpit-metrics">
      <article v-for="item in displayMetrics" :key="item.key">
        ...
        <em v-if="item.live" class="up">实时业务数据</em>
        <em v-else :class="item.trend">{{ item.trend === 'up' ? '↑' : '↓' }} {{ item.change }} 较上期</em>
      </article>
    </section>
    ...
    <div class="cockpit-grid bottom-grid">
      ...
      <section class="cockpit-panel region-ranking"><div class="cockpit-panel-head"><h2>区域经营排名</h2><span>{{ isCurrentPeriod ? '合同回款率' : '收入完成率' }}</span></div><button v-for="(item, index) in displayRegionPerformance" :key="item.name" type="button" @click="openRegion(item)"><b>{{ index + 1 }}</b><span><strong>{{ item.name }}</strong><small>{{ item.revenue.toLocaleString() }} 万元</small></span><em>{{ item.targetRate }}%</em></button></section>
      <section class="cockpit-panel health-panel"><div class="cockpit-panel-head"><h2>运营质量</h2><span>核心效率指标</span></div><div class="efficiency-list"><div v-for="item in displayEfficiencyMetrics" :key="item.label"><span>{{ item.label }}</span><strong>{{ item.value }}<small>{{ item.unit }}</small></strong><em>{{ item.change }}</em></div></div><div class="cockpit-alert"><el-icon><Warning /></el-icon><span>{{ displayRisks.filter((item) => item.level === '高').length }} 项高风险事项需要优先处置</span></div></section>
    </div>
    ...
    <el-drawer v-model="detailVisible" title="区域经营详情" size="430px">
      <div v-if="selectedRegion" class="region-detail"><h2>{{ selectedRegion.name }}</h2><el-descriptions :column="1" border><el-descriptions-item :label="isCurrentPeriod ? '累计合同金额' : '累计收入'">{{ selectedRegion.revenue.toLocaleString() }} 万元</el-descriptions-item><el-descriptions-item v-if="!isCurrentPeriod" label="收入增长">{{ selectedRegion.growth }}%</el-descriptions-item><el-descriptions-item label="收入贡献">{{ selectedRegion.share }}%</el-descriptions-item><el-descriptions-item :label="isCurrentPeriod ? '合同回款率' : '目标完成率'">{{ selectedRegion.targetRate }}%</el-descriptions-item></el-descriptions><el-progress type="dashboard" :percentage="selectedRegion.targetRate" /><p>区域经营整体保持增长，下一阶段重点提升高价值商机转化与合同回款效率。</p></div>
    </el-drawer>
  </div>
</template>
